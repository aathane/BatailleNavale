...
void fire_artillery(char tab_boat[10][11], char tab[10][11], char mode_de_jeu) {
    ...
    // Car tout au long de cette fonction nous allons printf où le joueur a touché un bateau pour le mode blinf
    if (mode_de_jeu == '2') {
        for(int j = 0; j<=10; j++){
            printf("\n \n \n \n \n");
        }
    ...
    for (int e = 1; e <= 10; e++) {
        if (tab_boat[ligne][e] == '+' || tab_boat[ligne][e] == 'x') {
            // si la coordonnée correspond à un bateau ( un '+')
            tab[ligne][e] = 'x';                                   // alors on modifie la grille pour indiquer au joueur qu'il a toucher un bateau
            tab_boat[ligne][e] = 'x';                              // et on modifie la grille virtuel "tab_boat"
            if(mode_de_jeu == '2'){
    ...
    for (int l = 0; l <= 9; l++) {
        if (tab_boat[l][colonne] == '+'|| tab_boat[l][colonne] == 'x' ) {
            tab[l][colonne] = 'x';
            tab_boat[l][colonne] = 'x';
            if(mode_de_jeu == '2' && l != ligne){
                printf("Touch%c en: %c%d\n",130, 65 + l, colonne);
            }
        } else{
                tab[l][colonne] = 'O';
                tab_boat[l][colonne] = 'O';
            }
        }
